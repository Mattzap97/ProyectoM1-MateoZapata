...
<li>Copia del nombre de un color al portapapeles, ya sea en formato HEX o HSL.</li>
<li>Mensajes con formato micro feedback que informan al usuario al generar una nueva paleta y copiar un color.</li>
...
<li>Diseño responsive sencillo para computadoras y celulares.</li>
...
3. Pulsar el botón "Generar Paleta" para crear colores aleatorios.
4. Debajo de cada tarjeta encontrarás su información dentro de un recuadro blanco visible, Pulsa sobre el color que desees para copiar su información.
5. Comprobar el mensaje en pantalla que te confirma la acción de copiar.
...
Dentro del archivo CSS se definió la estructura del proyecto, además de establecer un solo tipo de fuente para todo el texto que la conforma; con color blanco para que el usuario no presente dificultades al leer el texto que contiene la página, tanto en títulos, como enlaces internos y contenido de los botones.

El cuerpo de la página se estableció con un fondo de color oscuro para que las tarjetas de colores resalten y sean visibles al momento de generar colores aleatorios. La información del color tiene texto oscuro y un fondo de color blanco para ayudar a la accesibilidad del usuario y no tenga problemas en leer el código de cada color independientemente del formato seleccionado.
...
H: Hue (tono del color entre 0 y 359 grados)

S: Saturation (Saturación del color entre 60 y 100%)

L: Luminous (Luminosidad del color entre 35 y 65%)
...
Para guardar una paleta de colores se ocupa localStorage.setItem() y previamente haber definido un array vacío para que después de haber recorrido todos los colores. Al momento de guardar la paleta se puede comprobar en la consola que todos los colores guardados aparecen en localStorage respetando el número de colores y el formato seleccionado por el usuario.
...
Dentro de algún tiempo se realizará una actualización para que el usuario pueda visualizar la paleta guardada junto con sus respectivos colores.


Para el menú selector de números de colores a generar dentro de la paleta desde HTML se definió en un principio que la paleta llevaría los 9 colores con su respectivo recuadro de información debajo de cada color. Esto con la intención de que dentro de JavaScript resulte más fácil ocultar los colores en cuanto el usuario seleccione 8 o 6.
